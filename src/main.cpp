...
uint256 hashGenesisBlock("0x3a69179a4ab0a9324ecf915abfb717a14ac16a8479d8f8bf90c9e464526784b4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Janusz Pieniądz Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy /= 1 << (nHeight / (3 * 60 * 24 * 365));
    // half the reward every year
 
    if(nHeight < 0) // no block reward within the first day
        nSubsidy = 0;
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Janusz Pieniądz: 1 hour
static const int64 nTargetSpacing = 20; // Janusz Pieniądz: 20 seconds
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal januszpieniadz miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Janusz Pieniądz", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	// vMerkleTree: 5a2e19825b
        ...
        // Genesis block
        const char* pszTimestamp = "01.01.2014 karahen wlaczyl DNS-y. ps. tak bylo. ps2. sram hooyem";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388617671;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 171765;

        if (fTestNet)
        {   //testnet is not configured
            block.nTime    = 1388617671;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x09c9d98ca86aeff9f53d2763c5b8f2d6fc78b531ba4f22ebe80ea35e71ba24b6"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xd9, 0x21, 0x37 };
